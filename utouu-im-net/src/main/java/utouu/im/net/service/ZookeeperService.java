...
public class ZookeeperService extends BaseService implements IZookeeperService {
	...
	public void serverInit() {
		try {
			...
			ZookeeperTool.createZnode("/sunhua/online/" + address + "-" + port + "/online_accounts",
					CreateMode.EPHEMERAL, ZooDefs.Ids.OPEN_ACL_UNSAFE);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void updateOnlineAccounts(String onlinesKeysString) {
		try {
			ZookeeperTool.setData("/sunhua/online/" + address + "-" + port + "/online_accounts",
					onlinesKeysString.getBytes("UTF-8"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public AccountOnlineVO checkAccountOnline(String account) {
		...
		try {
			...
			for (String children : online_server_children) {
				...
				Map<String, Set<String>> map = JSONObject.parseObject(nodeOnlineAccountString,
	...
	public void destory() {
		try {
			System.out.println("########################链接销毁");
			ZookeeperTool.deleteZnode("/sunhua/online/" + address + "-" + port + "/online_accounts");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
